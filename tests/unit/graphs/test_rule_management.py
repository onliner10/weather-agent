...
from weather_agent.domain.rules.models import NotificationRule, RuleUpdate
...
class TestProposeCelRuleNode:
    ...
    async def test_llm_returns_null_expression(self) -> None:
        ...
        assert result["cel_expression"] is None
        assert result["error"] is not None
        assert "Nie da się" in result["error"]
    ...
    async def test_llm_failure(self) -> None:
        ...
        assert result["error"] is not None
        assert "Błąd modelu" in result["error"]
    ...
    async def test_malformed_llm_json(self) -> None:
        ...
        assert result["error"] is not None
        assert "Nie udało się przetworzyć" in result["error"]
...
class TestPersistRuleChangeNode:
    ...
    @pytest.mark.asyncio
    async def test_user_confirms_edit_rule(self) -> None:
        mock_rule = NotificationRule(
            id=1,
            short_id="R7K2",
            user_id=42,
        ...
        call_args = rule_service.update_rule.call_args
        assert call_args[0][0] == 1
        update_data = call_args[0][1]
        assert isinstance(update_data, RuleUpdate)
        assert update_data.expression == _VALID_CEL
