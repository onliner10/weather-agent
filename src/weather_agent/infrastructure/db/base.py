...
class AuthorizedUser(Base):
    ...
    notification_rules: Mapped[list[NotificationRule]] = relationship(back_populates="user")
...
class Location(Base):
    ...
    forecast_snapshots: Mapped[list[ForecastSnapshot]] = relationship(back_populates="location")
    forecast_points: Mapped[list[ForecastPoint]] = relationship(back_populates="location")
    observations: Mapped[list[Observation]] = relationship(back_populates="location")
    official_warnings: Mapped[list[OfficialWarning]] = relationship(back_populates="location")
    notification_rules: Mapped[list[NotificationRule]] = relationship(back_populates="location")
...
class TelegramContext(Base):
    ...
    __table_args__ = (Index("ix_telegram_contexts_chat_id", "chat_id"),)
...
class ForecastSnapshot(Base):
    ...
    forecast_points: Mapped[list[ForecastPoint]] = relationship(back_populates="snapshot")
    rule_evaluation_runs: Mapped[list[RuleEvaluationRun]] = relationship(back_populates="snapshot")
...
class ForecastPoint(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    snapshot_id: Mapped[int] = mapped_column(ForeignKey("forecast_snapshots.id"))
    ...
    precipitation_probability_pct: Mapped[float | None] = mapped_column(nullable=True)
    ...
    relative_humidity_2m_pct: Mapped[float | None] = mapped_column(nullable=True)
    weather_code: Mapped[str | None] = mapped_column(String(50), nullable=True)
    raw_payload: Mapped[dict[str, object]] = mapped_column()

    snapshot: Mapped[ForecastSnapshot] = relationship(back_populates="forecast_points")
...
class Observation(Base):
    ...
    __table_args__ = (
        Index("ix_observations_location_observed", "location_id", "observed_at"),
        Index(
...
class OfficialWarning(Base):
    ...
    location: Mapped[Location] = relationship(back_populates="official_warnings")
...
class NotificationRule(Base):
    ...
    telegram_message_thread_id: Mapped[int | None] = mapped_column(nullable=True)
    location_id: Mapped[int] = mapped_column(ForeignKey("locations.id"))
    expression_language: Mapped[str] = mapped_column(String(20), default="cel")
    ...
    cooldown_minutes: Mapped[int | None] = mapped_column(nullable=True, default=60)
    enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    ...
    user: Mapped[AuthorizedUser] = relationship(back_populates="notification_rules")
    location: Mapped[Location] = relationship(back_populates="notification_rules")
    notification_events: Mapped[list[NotificationEvent]] = relationship(back_populates="rule")
    rule_evaluation_runs: Mapped[list[RuleEvaluationRun]] = relationship(back_populates="rule")

    __table_args__ = (Index("ix_notification_rules_user_enabled", "user_id", "enabled"),)
...
class NotificationEvent(Base):
    ...
    telegram_chat_id: Mapped[int] = mapped_column(BigInteger)
    telegram_message_thread_id: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
...
class RuleEvaluationRun(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    rule_id: Mapped[int] = mapped_column(ForeignKey("notification_rules.id"))
    ...
    rule: Mapped[NotificationRule] = relationship(back_populates="rule_evaluation_runs")
    snapshot: Mapped[ForecastSnapshot | None] = relationship(back_populates="rule_evaluation_runs")
...
class AuditLog(Base):
    ...
    context_key: Mapped[str | None] = mapped_column(String(100), nullable=True)
    details: Mapped[dict[str, object]] = mapped_column()
    created_at: Mapped[datetime] = mapped_column(default=_utcnow)

    __table_args__ = (Index("ix_audit_log_event_created", "event_type", "created_at"),)
